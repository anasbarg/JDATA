...
def distribution(year, a, path):
    ...
    df_year = df[df.year==year] # a filtered dataframe that only contains data for the year of interest
    x= np.log10(df_year.AverageIncome/12) # the x-axis is log scale of the average monthly income in the year of interest
...
def share(p_start, p_end, Year_from, Year_to, path):
    df = pd.read_csv(path) # a dataframe that forms the main source of data
    df_share1 = df[(df.year>=Year_from)&(df.year<=Year_to)&(df.start>=p_start)&(df.start<p_end)].copy()#filter required population and time period
...
def Threshold_range(p_start,p_end, Year_from,Year_to, path):
    df = pd.read_csv(path) # a dataframe that forms the main source of data
    df_threshold1 = df[(df.year>=Year_from)&(df.year<=Year_to)&(df.start>=p_start)&(df.start<=p_end)].copy()#filter required population and time period
    df_threshold1 = df_threshold1.groupby('year') # calculate share
    aggregation = {'Threshold(JOD)':{'threshold_min' : 'min','threshold_max' : 'max'}}
