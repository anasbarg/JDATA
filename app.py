...
@app.route("/api/rank/<float:salary>/<int:year_from>/<int:year_to>")
def rank_data(salary, year_from, year_to):
    indecies = [index for index, row in data.percentile(
        salary, year_from, year_to, data_path).iterrows()]
    ranks = [row["start"]
             for index, row in data.percentile(
        salary, year_from, year_to, data_path).iterrows()]
    points = [list(i) for i in zip(indecies, ranks)]
    datasets = {"rank": points}
    response = app.response_class(
        response=json.dumps(datasets),
        status=200,
        mimetype='application/json'
